...
define(['jquery', 'core/templates', 'core/ajax', 'core/notification', 'core/str', 'theme_remui/feedbackcollection'], function($, Templates, Ajax, Notification, Str, feedbackcollection) {
    ...
    function licenseActivationHandler($licensekey, action = "check") {
        Ajax.call([{
            ...
            args: {
                ...
                if (response['alert']) {
                    $(SELECTORS.LICENSEACKMSG).removeClass("d-none").removeClass("sucessinfo failureinfo").addClass(textinfoclass).text(response['alert']['text']);
    ...
    const registerCommonEvents = (showmodal) =>{
        ...
        $(document).on("click", SELECTORS.CONTINUESETUPBTN, function(){
    ...
    function init(isremui = false) {
        $(document).ready(async function(){
            ...
            $(document).on('submit', SELECTORS.USERINFORMATION_FORM, submitUserInformation);
